...
void _uart0_receive(void)
{
	unsigned char * recpcdata;
	unsigned char temp_serial_data;
	recpcdata = &pc_recd_buffer.dest_addr;
	temp_serial_data = u0rb;
	if (pc_data_recd == false)
	{
		switch (pc_data_ctr)
		{
			case 0:
				if (temp_serial_data == SOURCE_UNIT_ADDRESS)
				{
					recpcdata[pc_data_ctr] = temp_serial_data;
					pc_data_ctr = 1;
					pending_pcdata_bytes = 0;
				}
				break;
			case 1:
				recpcdata[pc_data_ctr] = temp_serial_data;
				pending_pcdata_bytes = temp_serial_data + 3;
				pc_data_ctr = 2;
				break;
			default:
				if (pending_pcdata_bytes > 0)
				{
					recpcdata[pc_data_ctr] = temp_serial_data;
					pc_data_ctr++;
					pending_pcdata_bytes--;
					if (pending_pcdata_bytes == 0)
					{
						pc_data_recd = true;
						pc_data_ctr = 0;
...
void _timer_ra(void)
{
	...
	if (half_sec_ctr == 0)
	{
		...
		{
			key_timeout_ctr--;
			if (key_timeout_ctr == 0)
			{
				key_timeout_flag = false;
			}
		}
	}
	if (pc_timeout_flag == true)
	{
		pc_timeout_ctr--;
		if (pc_timeout_ctr == 0)
		{
